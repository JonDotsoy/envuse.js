...
describe("Sample 1", () => {
  it("should prepare envuse file", async () => {
    ...
    expect(
      createDTSFile(await createProgramFromFile(envuseFile), "demo")
        .toString()
...
describe("Sample 2: store of types", () => {
  it("create store type reference", async () => {
    ...
    const storeTypeReference = createStoreTypeReference(definitionURL, typesURL)
    ...
    storeTypeReference.attachInterfaceDTSFile(new InterfaceDTSFile("a", [
      new FieldDTSFile("name", new PrimitiveStringTypeDTSFile()),
      new FieldDTSFile("age", new UnionDTSFile([new PrimitiveStringTypeDTSFile(), new PrimitiveNumberTypeDTSFile()])),
    ]))
    storeTypeReference.attachInterfaceDTSFile(new InterfaceDTSFile("c", []))
    storeTypeReference.attachInterfaceDTSFile(new InterfaceDTSFile(".envuse", []))
    storeTypeReference.attachInterfaceDTSFile(new InterfaceDTSFile(".envuse.prod", []))
    storeTypeReference.attachInterfaceDTSFile(new InterfaceDTSFile(".envuse/dev.env1", []))
    storeTypeReference.attachInterfaceDTSFile(new InterfaceDTSFile(".envuse/prod.env", []))
...
it("Should parser file", async () => {
  const testDirname = new URL("_sample-3/", import.meta.url)
  const definitionURL = new URL("def.json", testDirname)

  const a = await parseAsync("sample.envuse")
})
