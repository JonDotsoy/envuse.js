...
export class ExportDTSFile extends DTSFile {
    constructor(readonly expression: DTSFile) { super() }
    * toLines(depth: number): Generator<NewType> { yield [depth, `export ${this.expression.toString(depth)}`] }
}
export class KeywordDTSFile extends DTSFile {
    constructor(readonly raw: string) { super() }
    * toLines(depth: number): Generator<NewType> { yield [depth, this.raw.replace(/\W/g, '_')] }
}
export class StringDTSFile extends DTSFile {
    constructor(readonly raw: string) { super() }
    * toLines(depth: number): Generator<NewType> { yield [depth, JSON.stringify(this.raw)] }
}
export class AnyTypeDTSFile extends DTSFile {
    * toLines(depth: number): Generator<NewType> { yield [depth, `any`] }
}
export class UndefinedTypeDTSFile extends DTSFile {
    * toLines(depth: number): Generator<NewType> { yield [depth, `undefined`] }
}
export class NullTypeDTSFile extends DTSFile {
    * toLines(depth: number): Generator<NewType> { yield [depth, `null`] }
}
export class UnionDTSFile extends DTSFile {
    constructor(readonly types: [DTSFile, ...DTSFile[]]) { super() }
    * toLines(depth: number): Generator<NewType> {
...
export class PrimitiveStringTypeDTSFile extends DTSFile {
    * toLines(depth: number): Generator<NewType> { yield [depth, `string`] }
}
export class PrimitiveNumberTypeDTSFile extends DTSFile {
    * toLines(depth: number): Generator<NewType> { yield [depth, `number`] }
}
export class PrimitiveBooleanTypeDTSFile extends DTSFile {
    * toLines(depth: number): Generator<NewType> { yield [depth, `boolean`] }
}
...
export class FieldDTSFile extends DTSFile {
    ...
    constructor(readonly fieldName: string, readonly fieldValue: DTSFile) {
        super()
        this.safeFieldName = /\W/.test(fieldName) ? new StringDTSFile(fieldName) : new KeywordDTSFile(fieldName)
    }

    * toLines(depth: number): Generator<NewType> {
        yield [depth, `${this.safeFieldName}: ${this.fieldValue.toString(depth).trimStart()}`]
    }
}
