...
export const createProgram = (source: string, location?: string): Program => envuse.create_program(source, location)
...
export const createDTSFile = (program: Program, interfaceName: string): InterfaceDTSFile => {
    const interfaceResult = new InterfaceDTSFile(interfaceName, []);

    for (const element of program.ast.Document.elements) {
        const isVariable = (elm: unknown): elm is { Variable: Variable } => typeof elm === "object" && elm !== null && "Variable" in elm
        ...
        if (isVariable(element)) {
            ...
            const dtsTypeValue = (() => {
                switch (variable_type) {
                    case "boolean": return new PrimitiveBooleanTypeDTSFile();
                    case "number": return new PrimitiveNumberTypeDTSFile();
                    case "string": return new PrimitiveStringTypeDTSFile();
                    default: return new AnyTypeDTSFile();
                }
            })();
...
export const createStoreTypeReference = (definitionURLLike: string | URL, typesURLLike: string | URL) => {
